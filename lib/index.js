const pluckJson = (path, data) =>
    data && path.split(".")
        .reduce((prev, curr) => prev == null ? null : prev[curr], data);
 ...
 */
const mapData = module.exports = (expr, data, config = null) => {
...
mapData.asInt = (selector, defaultValue = NaN) => ({
    _selector: selector,
    _transform: value => isNaN(value) ? defaultValue : parseInt(value)
});

mapData.coalesce = (selector, defaultValue) => ({
    _selector: selector,
    _transform: value => value == null ? defaultValue : value
});